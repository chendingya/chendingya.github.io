---
title: 软件系统设计-原则模式期末复习
date: 2024-06-24 10:11:44
tags: 
 系统架构
categories: 
 软件系统设计
...
[UML-认识六种箭头，轻松读懂UML图_uml图六种箭头的含义-CSDN博客](https://blog.csdn.net/qq_25091281/article/details/123801862)
...
![img](软件系统设计期末复习-原则模式/1718712093864-4038f724-6fcd-4bb2-be80-72a90b79b5b1.png)
...
![img](软件系统设计期末复习-原则模式/1718714151414-addf888a-0ac8-4c4d-9542-88084067fcb9.png)
...
![img](软件系统设计期末复习-原则模式/1718714151299-16b744ef-26c8-4ffc-8776-87b1d794f161.png)
...
![img](软件系统设计期末复习-原则模式/1718714257371-fe9356a6-2cc1-4795-be79-65d9fa14b274.png)

![img](软件系统设计期末复习-原则模式/1718714257555-b6fdb885-dcdd-429b-adf5-83670ce558c4.png)

现对该系统进行重构，使之满足开闭原则的要求：尝试将代码变为数据（配置文件，Config.xml），结合 Java 的反射机制来完成。

![img](软件系统设计期末复习-原则模式/1718714257452-90da64bf-8ad3-45bd-9eb8-472276f1c288.png)
...
![img](软件系统设计期末复习-原则模式/1718714338794-d79a7952-1224-41ef-a1f0-7eb967e40748.png)

也可以为 CipherA 和 CipherB 设计一个共同的父类，下面是指 CipherB 是在 CipherA 的基础上加密。

![img](软件系统设计期末复习-原则模式/1718714338902-8d00762b-3cfa-4362-8bb7-74b6aa7a01db.png)
...
2. 代码要依赖于抽象的类，而不要依赖于具体的类；要针对接口或抽象类编程，而不是针对具体类编程。
3. 依赖注入

1. 构造注入（Constructor Injection）：通过构造函数注入实例变量。
2. 设值注入（Setter Injection）：通过 Setter 方法注入实例变量
3. 接口注入（Interface Injection）：通过接口方法注入实例变量。

![img](软件系统设计期末复习-原则模式/1718973825334-9fb2bd75-8347-4db0-be26-b2ab5454ac9c.png)

把高层模块和低层模块交互行为抽象出来，做到互不依赖，都依赖于提取出来的抽象。

**即：无论高层模块或低层模块实现的细节，都依赖于抽象**。

![img](软件系统设计期末复习-原则模式/1718973893916-e5e77934-d0ee-403b-af62-6ed3462f0dec.png)
...
![img](软件系统设计期末复习-原则模式/1718714410934-c50062f4-6fb5-45cd-af2e-523989c03b70.png)

由于需求的变化，该系统可能需要增加新的数据源或者新的文件格式，每增加一个新的类型的数据源或者新的类型的文件格式，客户类 MainClass 都需要修改源代码，以便使用新的类，但违背了开闭原则。现使用依赖倒转原则对其进行重构。

![img](软件系统设计期末复习-原则模式/1718714410916-a517a9e9-ab2e-4cef-9bc0-2dfa92487254.png)
...
![img](软件系统设计期末复习-原则模式/1718721703771-45748098-5f01-4524-80ea-f124374a3d89.png)

![img](软件系统设计期末复习-原则模式/1718721703757-a02ded6a-f107-4135-acdb-586539a7519e.png)
...
![img](软件系统设计期末复习-原则模式/1718721841599-45c5342c-63b9-4f7e-8447-1846f2c1ed33.png)

1. 如果需要更换数据库连接方式，如原来采用 JDBC 连接数据库，现在采用数据库连接池连接，则需要修改 DBUtil 类源代码。如果 StudentDAO 采用 JDBC 连接，但是 TeacherDAO 采用连接池连接，则需要增加一个新的 DBUtil 类，并修改 StudentDAO 或 TeacherDAO 的源代码，使之继承新的数据库连接类，这将违背开闭原则，系统扩展性较差。
2. 现使用合成复用原则对其进行重构。

![img](软件系统设计期末复习-原则模式/1718721841793-0789135c-9382-42db-bb0f-02259946a125.png)
...
4. 简单地说，迪米特法则就是指一个软件实体应当尽可能少的与其他实体发生相互作用。
5. 在迪米特法则中，对于一个对象，其朋友包括以下几类：
...
1. 迪米特法则可分为狭义法则和广义法则。在狭义的迪米特法则中，如果两个类之间不必彼此直接通信，那么这两个类就不应当发生直接的相互作用，如果其中的一个类需要调用另一个类的某一个方法的话，可以通过第三者转发这个调用。下图中，只允许 A 调用 B 对象的方法，但是不能调用 C 对象的方法（但是我们可以通过在 B 中添加一个 Wrapper 方法来间接调用 C）

![img](软件系统设计期末复习-原则模式/1718722141389-7c67911b-1960-4ceb-8493-785db9343f5c.png)
...
![img](软件系统设计期末复习-原则模式/1718722223447-3c59c473-b9e0-4037-938b-ff033de0ff73.png)

![img](软件系统设计期末复习-原则模式/1718722223556-67ce11ac-5739-4042-92ae-4e3ec0c727d7.png)
...
1. 根据范围，即模式主要是用于处理类之间关系还是处理对象之间的关系，可分为类模式和对象模式两种：
...
![img](软件系统设计期末复习-原则模式/1718784290700-3e523774-f637-426e-af36-b6d9d70795a5.png)

## 策略模式 ：行为型模式、对象模式

![img](软件系统设计期末复习-原则模式/1718784402761-e201e33b-5ab9-4240-b09c-46221f4fc995.png)
...
public class BulkPurchaseDiscount implements DiscountStrategy {
    public double applyDiscount(double originalPrice) {
        return originalPrice - 50; // $50 discount for bulk purchase
    }
}
```
...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("Total without discount: " + cart.calculateTotal());

        cart.setDiscountStrategy(new SeasonalDiscount());
        System.out.println("Total with seasonal discount: " + cart.calculateTotal());

        cart.setDiscountStrategy(new BulkPurchaseDiscount());
        System.out.println("Total with bulk purchase discount: " + cart.calculateTotal());
    }
}
```
...
![img](软件系统设计期末复习-原则模式/1718798659632-306266f5-7a64-4115-a77b-ad47d58157f1.png)

### 主要角色：

1. **环境（Context）**：维护一个当前状态类的实例，这个实例定义了当前的状态。
2. **状态接口（State）**：定义所有具体状态的共同接口。
3. **具体状态（Concrete State）**：实现状态接口，定义了对象在特定状态下的行为。
...
![img](软件系统设计期末复习-原则模式/1718799988786-d3ed36f1-6dee-4c8b-8aab-1a3413eb08b1.png)
...
![img](软件系统设计期末复习-原则模式/1718784499605-62969586-90db-4653-b921-acfd6d0d2ed4.png)
...
![img](软件系统设计期末复习-原则模式/1718785176924-83bf373f-4966-45b4-b990-ffa02ba9a189.png)



称为工厂模式，也叫虚拟构造器（Virtual Constructor）模式或者多态工厂（Polymorphic Factory）模式，它属于类创建型模式。

1. **抽象工厂（Abstract Factory）**：声明一个工厂方法，该方法返回一个产品类型的对象。
2. **具体工厂（Concrete Factory）**：实现抽象工厂中定义的工厂方法，创建并返回具体产品对象。
3. **抽象产品（Product）**：定义产品的接口。
4. **具体产品（Concrete Product）**：实现产品接口的类。
...
![img](软件系统设计期末复习-原则模式/1718785618288-dddeddd7-44d3-4420-a780-0db8823d24c9.png)
...
![img](软件系统设计期末复习-原则模式/1718785678952-d27afd23-827e-40cd-bcaa-a8c93f4103e4.png)
...
![img](软件系统设计期末复习-原则模式/1718787102009-3ea9d1e8-62b5-4745-8a64-3fbf5ebe6969.png)

将一个复杂对象的构建与它的表示分离，使得同样的构建过程可以创建不同的表示。

1. **产品（Product）**：要构建的复杂对象。
2. **建造者（Builder）**：为创建一个产品对象的各个部件提供接口。
3. **具体建造者（Concrete Builder）**：实现建造者接口以构造和装配产品的各个部分。
4. **导演（Director）**：构造一个使用建造者接口的对象。
5. **客户端（Client）**：使用建造者创建对象。
...
public class BuilderPatternDemo {
    public static void main(String[] args) {
        ...
        System.out.println("Dell Computer: " + dellComputer.getCpu() + ", " + dellComputer.getRam() + ", " + dellComputer.getHdd());

        ComputerBuilder hpBuilder = new HpComputerBuilder();
        Computer hpComputer = director.construct(hpBuilder);
        System.out.println("HP Computer: " + hpComputer.getCpu() + ", " + hpComputer.getRam() + ", " + hpComputer.getHdd());
    }
}
```
...
![img](软件系统设计期末复习-原则模式/1718787359300-abefd74f-1151-4ad0-b46b-d73bd84627ff.png)
...
1. 通常情况下，一个类包含一些成员对象，在使用原型模式克隆对象时，根据其成员对象是否也克隆，原型模式可以分为两种形式：深克隆和浅克隆。
2. Java 语言提供的 clone() 方法将对象复制了一份并返回给调用者。一般而言，clone() 方法满足:
...
![img](软件系统设计期末复习-原则模式/1718798049081-db381852-61a8-41b7-b931-4276ff9a0554.png)
...
![img](软件系统设计期末复习-原则模式/1718801390491-16def183-3489-4794-b399-aeaddb0c9c19.png)
...
![img](软件系统设计期末复习-原则模式/1718852345899-0979e70e-e758-4d7f-b2b4-e72fba93e84b.png)
...
public class CommandPatternDemo {
    public static void main(String[] args) {
        ...
        remoteControl.pressUndoButton(); // 输出：Light is on.
    }
}
```
...
![img](软件系统设计期末复习-原则模式/1718854730857-9f4a0554-cd3f-447d-8444-be32ee052190.png)

### 主要角色：

1. **主题（Subject）**：定义了注册观察者对象和注销观察者对象的方法，以及通知所有观察者对象的方法。
2. **具体主题（Concrete Subject）**：实现主题接口，并通常维护一个观察者对象的集合。
3. **观察者（Observer）**：定义了更新观察者的接口，通常会定义一个更新方法，用于接收主题的通知。
4. **具体观察者（Concrete Observer）**：实现观察者接口，通常会维护一个与主题对象关联的状态，并在接收到通知时更新自己的状态。
...
```java
public class ConcreteMailSystem implements MailSystem {
    private List<Observer> observers;
    private String message;

    public ConcreteMailSystem() {
...
public class User implements Observer {
    ...
    public void update(String message) {
        System.out.println("User " + name + " received a new message: " + message);
    }
}
```
...
public class ObserverPatternDemo {
    public static void main(String[] args) {
        MailSystem mailSystem = new ConcreteMailSystem();
        User user1 = new User("User 1");
        User user2 = new User("User 2");

        mailSystem.addObserver(user1);
        mailSystem.addObserver(user2);

        mailSystem.setMessage("New email received!");
    }
}
```
...
![img](软件系统设计期末复习-原则模式/1718861129423-fe60fb92-2fc9-432d-8647-76e2875892de.png)

![img](软件系统设计期末复习-原则模式/1718861168944-350dff17-5b3c-4521-915a-4efe1a21636a.png)

### 主要角色：

1. **中介者（Mediator）**：定义了所有同事对象之间的通信接口，通常包含一个方法，用于处理同事对象之间的交互。
2. **具体中介者（Concrete Mediator）**：实现中介者接口，通常维护对所有同事对象的引用，并负责协调它们之间的交互。
3. **同事（Colleague）**：定义了与中介者进行通信的方法，并通常包含一个对中介者的引用。
4. **具体同事（Concrete Colleague）**：实现同事接口，并通常包含一个对中介者的引用。
...
![img](软件系统设计期末复习-原则模式/1718861294865-f39634e5-a3ac-4392-853f-c7ae4a964d75.png)
...
![img](软件系统设计期末复习-原则模式/1718864914594-d60f3bea-8026-4382-9172-7492c9b25103.png)

1. 模板方法模式是一种类的行为型模式，在它的结构图中只有类之间的继承关系，没有对象关联关系。
2. 抽象方法（Abstract Method）
3. 具体方法（Concrete Method）
4. 钩子方法（Hook Method）：“挂钩”方法和空方法

1. 钩子方法的使用

1. 钩子方法的引入使得子类可以控制父类的行为。
2. 最简单的钩子方法就是空方法，也可以在钩子方法中定义一个默认的实现，如果子类不覆盖钩子方法，则执行父类的默认实现代码。
3. 比较复杂一点的钩子方法可以对其他方法进行约束，这种钩子方法通常返回一个 boolean 类型，即返回 true 或 false，用来判断是否执行某一个基本方法。
...
![img](软件系统设计期末复习-原则模式/1718866639319-3e6fec7e-2d55-4fbc-a3c6-38b0fd0668f0.png)
...
1. 接口相关模式：适配器模式、外观模式，变化决定了要使用哪个模式
2. 外观模式包含如下角色：
...
![img](软件系统设计期末复习-原则模式/1718867658731-96bb4c87-e4c6-4123-acda-460e2260d47c.png)
...
public class FacadePatternDemo {
    public static void main(String[] args) {
        ...
        officeFacade.printAndSave(); // 输出：Printing..., Scanning..., Saving to file...
    }
}
```
...
现在考察一个电源总开关的例子，以便进一步说明外观模式。为了使用方便，一个电源总开关可以控制四盏灯、一个风扇、一台空调和一台电视机的启动和关闭。通过该电源总开关可以同时控制上述所有电器设备，使用外观模式设计该系统。![img](软件系统设计期末复习-原则模式/1718867783204-7600aa40-9523-4529-90c1-b628c19cd870.png)
...
![img](软件系统设计期末复习-原则模式/1718868037660-519e6f08-daf3-461c-a2e9-b6684c14f0eb.png)
...
![img](软件系统设计期末复习-原则模式/1719132251077-471106a4-8515-4e8f-81d1-bbbca62e1d7c.png)

### 对象适配器

![img](软件系统设计期末复习-原则模式/1718869196945-c062a59c-cd74-47e6-ab74-e96e1422ade6.png)

### 类适配器

![img](软件系统设计期末复习-原则模式/1718869220555-0ea03409-9563-46d0-9af5-2625ca3ba175.png)
...
![img](软件系统设计期末复习-原则模式/1718871852913-70c55fb6-196c-48b2-8f36-dc4f73ff4be9.png)
...
public class SimpleRobot implements Robot {
    @Override
    public void cry() {
        System.out.println("Robot: Crying...");
    }
...
// 狗行为的适配器类
public class DogBehaviorAdapter implements Robot {
    private Dog dog;

    public DogBehaviorAdapter(Dog dog) {
        this.dog = dog;
    }
...
public class AdapterPatternDemo {
    public static void main(String[] args) {
        ...
        Dog dog = new Dog();
        Robot dogBehaviorAdapter = new DogBehaviorAdapter(dog);
        dogBehaviorAdapter.cry();
        dogBehaviorAdapter.move();
    }
}
```
...
![img](软件系统设计期末复习-原则模式/1718871992679-6bba1a14-0c31-46a4-93eb-fd0b55fe35fe.png)
...
![img](软件系统设计期末复习-原则模式/1718873162727-43036224-a57e-47f3-bd78-28307b8f999c.png)
...
2. 开闭原则支持好：无需更改现有代码， 你就可以在应用中添加新元素， 使其成为对象
   树的一部分。
3. 违反最小接口原则：leaf和composite都会继承一些不属于自己的方法
...
![img](软件系统设计期末复习-原则模式/1718885407344-0af6a296-4bdc-4696-9acc-43aa64712747.png)

文件有不同类型，不同类型的文件其浏览方式有所区别，如文本文件和图片文件的浏览方式就不相同。对文件夹的浏览实际上就是对其中所包含文件的浏览，而客户端可以一致地对文件和文件夹进行操作，无须关心它们的区别。使用组合模式来模拟文件的浏览操作。

![img](软件系统设计期末复习-原则模式/1718885624251-5a39bb45-8af8-4dfa-b3d5-d7df478362a9.png)
...
![img](软件系统设计期末复习-原则模式/1718885950184-8b8eb44c-88bf-4bba-8ff8-6abf9ee2a712.png)



### 主要角色：

1. **抽象构件（Component）**：定义了对象接口，可以包含一个或多个方法。
2. **具体构件（ConcreteComponent）**：实现了抽象构件接口，通常由具体的对象实现。
3. **抽象装饰（Decorator）**：继承自抽象构件，用于定义装饰器接口。
4. **具体装饰（ConcreteDecorator）**：继承自抽象装饰，并实现抽象装饰接口。具体装饰通常包含一个具体构件对象，并添加额外的功能。
...
![img](软件系统设计期末复习-原则模式/1718890661555-82f92ab0-2561-46de-8e2f-26d3e5103f8b.png)
...
1. 【2023】为什么最小知识原则可以帮助构建高内聚、低耦合的系统？用代码举一个违背的例子
...
class Order {
    ...
    public double calculateTotalPrice() {
        // 违背了最小知识原则，因为Order对象直接访问了Customer对象的内部数据
...
1. 【2022】【2023】What is the benefit of decoupling the Receiver from the Invoker in the Command Pattern?
...
1. 【2023】设计一个系统，有很多的机器人，他们有不同的功能，在未来会拓展这些功能或功能的复杂性，同时有一批保留了旧接口的老机器人，也希望能加入到这个系统中。系统还能将所有的机器人看作一个 unit 来统一的控制。1. 列出使用的设计模式和类图 2.说明你的设计如何有利于未来拓展功能
...
1. 【2022】【2023】观察者设计模式中有两种方法用于向观察者传播数据：推模型（Push Model）和拉模型（Pull Model）。为什么有些情况下一个模型会比另一个模型更可取？每个模型的权衡是什么？
