...
1. 方便交流，软件开发过程中，**子任务与人员**之间存在错综复杂的关系，存在大量的沟通和交流，所以要在软件开发中要编写不同类型的文档，每种文档都是针对项目中需要广泛交流的内容。因为**软件需求**需要进行**广泛交流**，所以要把需求文档化。
2. 需求规格说明是在**软件产品的角度以系统级需求列表的方式**描述软件系统解决方案，书写需求规格说明，可以建立管理控制的基线，方便任务分配，制定工作计划，进行跟踪和度量。
3. 在实验中，需求规格的重要性不只体现在结果上，还包括中间过程，在书写需求规格过程中，才真正把问题域的问题和分析模型的成果转化为系统级需求，方便小组成员真正明确需求，个人认为在这个阶段**包含一部分的需求再发现和完整化**。
...
1.  在软件产品的角度以系统级需求列表的方式描述软件系统解决⽅案

![](https://spricoder.oss-cn-shanghai.aliyuncs.com/2020-Software-Engineering-and-Computing-II/img/cpt7/2.png)

### 3.3.1. 实例
1.  在一栋大楼中有一个电梯，现在有⼈打算从1楼使用电梯到达x楼。
    + 请写出其用例的场景描述
    + 根据用例场景编写SRS片段
    
2. 用例的场景描述:(其实从1层到x层作为用例不合适，**应该把整个上行过程作为用例**)
    1. 人在1楼按按钮，等待电梯(系统调度图)
    2. 电梯到达，电梯开门，人登上电梯
    3. 人按x楼的按钮，系统关门
    4. 电梯启动开始上升
    5. 人到达x楼，电梯开门
...
4.  SRS片段：
    ...
    - 业务需求：系统能正常运行，实现大楼人员的上下楼

    - 用户需求：系统要帮助人上楼

    - 系统级需求：

        - 在人在一楼按了按钮后，电梯应该能运行到一楼并开门
        - 在人进入电梯按了x楼按钮后，电梯应该关门并运行到x楼并开门
    ...
    - 相关功能需求：（**应当和刺激响应当一致对应**，描述相关功能需求时也应该是**描述刺激**，然后**描述响应**）

        Lift.Button 系统应该允许用户按电梯按钮

        Lift.Button.Outside 用户在电梯外按按钮，电梯运行到1楼并开门

        Lift.Button.Outside.Same 用户在电梯到达1楼开门后按按钮，系统不予处理

        Lift.Button.Inside 用户在电梯内按x楼按钮，电梯运行x楼并开门

        Lift.Button.Inside.Same 用户在电梯到达x楼开门后按x楼的按钮，系统不予处理

        Lift.Button.Inside.More 用户在电梯内按了多个按钮，电梯对应楼层按钮应该全部亮起，并按顺序运行到相应楼层后开门
...
1. "After the payment process is complete, the **relevant** information should be appended to a log ﬁle."(b)
    ...
    + b. This requirement should be rewritten; it is ambiguous or inconsistent. 模糊的（没有说明相关的是什么）
...
2. "The system should be constructed so that it will be **easy** to add new functionality in the future."(d)
    ...
    + c. This requirement should be rewritten; it is unrealistic.（easy是不容易验证的，怎么验证容易加入）
...
- 在**需求开发完成之后，测试人员就可以以需求为基础开发系统测试用例（主要是功能测试）了**。
- **这些测试用例将==在软件系统实现之后的测试阶段得到执行==**。
- 在实践中发现，在为需求开发测试用例的过程中可以发现软件需求规格说明文档的缺陷与问题。
- 因此，**为需求开发测试用例也被看做有效的需求验证方法**。
...
4. 软件需求数量
5. ⾮功能需求数量
6. 功能点数量：功能点不是功能，功能点数不是功能数。功能点是[Albrecht! 979] 提出的用于估算和度量软件系统规模与复杂度的抽象单位。在需求开发阶段，估计代码行数误差较大，使用功能点来估算和度量软件系统的规模与复杂度则有较好的效果。
