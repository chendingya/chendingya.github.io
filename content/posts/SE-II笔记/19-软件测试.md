...
- 把所有可能的输入数据，即程序的输入域划分为若干部分(子集)，**从每个子集中选取少量具有代表性的数据**作为测试用例。
- 例子
...
3. 边界值分析不是从某等价类中随便找一个作为代表，而是这个**等价类的每个边界都要作为测试条件**。
...
### 7.4.1. 白盒测试方法-语句覆盖
1. **语句覆盖设计测试用例**的标注是保证被测试的对象的**每一行代码都至少执行了一次。**
